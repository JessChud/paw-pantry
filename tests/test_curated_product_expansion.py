...
def test_reviewed_amazon_expansion_is_complete_and_synchronized():
    ...
    assert len(expansion) == 975
    assert len({row[0] for row in expansion}) == 975
    ...
    expansion_products = [product for product in products if 31 <= product['id'] <= 1005]
    assert len(expansion_products) == 975
